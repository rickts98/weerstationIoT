...
void ledControlOn(int led) { digitalWrite(PINARRAY[led], HIGH); }

// zet de led uit die meegegeven word
void ledControlOff(int led) { digitalWrite(PINARRAY[led], LOW); }

// returned aantal leds
int getNrLeds() { return NLEDS; }

// pas de status van de led aan in de araay ledStatus
void setLedStatus(int led, boolean status) { ledStatus[led] = status; }
